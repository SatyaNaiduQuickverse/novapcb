...
def collect_components():
    ...
    for fp in board.GetFootprints():
        ref = fp.GetReference()
        p = fp.GetPosition()
        # Skip parked components (X >= 100 mm — staged-build convention)
...
def check_decoupling(items):
    """For each IC, find caps within 3mm of the IC BODY EDGE
    (not center) — accounts for large QFP/QFN bodies where caps
    naturally sit at the edge."""
    bad = []
    for ref, d in items.items():
        if not (ref.startswith("U") and ref[1:].isdigit()):
            continue
        bb = d["fp"].GetBoundingBox()
        ic_x1, ic_y1 = pcbnew.ToMM(bb.GetLeft()), pcbnew.ToMM(bb.GetTop())
        ic_x2, ic_y2 = pcbnew.ToMM(bb.GetRight()), pcbnew.ToMM(bb.GetBottom())
        nearby = False
        for cref, cd in items.items():
            if not (cref.startswith("C") and cref[1:].isdigit()):
                continue
            cx, cy = cd["x"], cd["y"]
            # Distance from cap center to nearest IC body edge
            dx = max(ic_x1 - cx, 0, cx - ic_x2)
            dy = max(ic_y1 - cy, 0, cy - ic_y2)
            if math.hypot(dx, dy) <= 3.0:
                nearby = True
                break
        if not nearby:
            bad.append((ref, d["x"], d["y"]))
    if bad:
        fails.append(f"DECOUPLING: {len(bad)} ICs have no cap within 3mm of body edge")
        for r, x, y in bad[:10]:
            fails.append(f"  {r} at ({x:.1f},{y:.1f}) — no C within 3mm of body edge")
...
# check_symmetry skipped — ESC-specific CH1-4 pattern, not applicable to novapcb (use #22 symmetry task for A subsystem mirror check)
