#!/usr/bin/env python3
"""Apply CAN wires from Freerouting's t3_routing.ses to the board (task #45).

Bypass KiCad's ImportSpecctraSES (returns False on stripped DSN per
project pattern). Parse SES manually for the +5V net, add their
...
SES = os.path.join(HERE, "d4_core.ses")

D2_NETS = {"VCAP1","+3V3A","VREF_P","VBAT","BOOT0"}
...
def parse_ses_nets(ses_path):
    ...
    i = 0
    while True:
        m = re.search(r'\(net\s+"?([A-Za-z0-9_.+\-]+)"?', net_out[i:])
        if not m:
            break
        start = i + m.start()
        name = m.group(1)
...
def main():
    print(f"=== Apply D2 +5V wires from {SES} ===\n")
    if not os.path.exists(SES):
        print(f"!!! SES missing: {SES}")
        return 1

    all_nets = parse_ses_nets(SES)
    d2_in_ses = {n: segs for n, segs in all_nets.items() if n in D2_NETS}
    print(f"Found {len(d2_in_ses)} of {len(D2_NETS)} +5V net in SES")
    for name in sorted(D2_NETS):
        if name in d2_in_ses:
            segs = d2_in_ses[name]
    ...
    n_tr = 0; n_vi = 0
    for name, segs in d2_in_ses.items():
