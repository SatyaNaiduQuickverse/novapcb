...
| **M** | **MILESTONE — thermal architecture resolved at 105×85 mm, LDO kept** ⚠ **[THERMAL CLAIM RESCINDED 2026-05-23 — see HALT section below]** | branch `sim/gate12-v3-perbody`, PR #71, sha `874de12` | 2026-05-23 | 12 v3 (energy-balance + min-mesh-density PERMANENT gates), 13 | ⚠ **MCU=73.98°C / +6.02°C margin claim INVALIDATED**: the sweep used PLANNED component positions, not actual placement; actual placement gives MCU=82.46°C (over 80°C ceiling). LDO→buck escalation RE-OPENED. See `docs/THERMAL_ARCHITECTURE_DECISION.md` + OPEN_QUESTIONS phase5-thermal-ldo-vs-buck. — Original entry preserved verbatim: Master sign-off after gate12 v3 refactor (per-body Body Force replaces mesh-divergent MATC bbox; total injected power EXACT regardless of mesh). STEP4 regression: T_MCU spread 0.57°C across 4 meshes, energy balance +0.3% all meshes. Board-size sweep with rigorous powers (MCU=0.700W, U2=0.642W, Q5=0W hot-case, total 1.717W): smallest size with ≥5°C MCU margin = **105×85 mm** (MCU=73.98°C, +6.02°C margin). LDO→buck escalation CLOSED — `OPEN_QUESTIONS.md phase5-thermal-ldo-vs-buck`. Outline + mounting holes updated; DECISIONS.md §2 records the full evolution. STEP4's 80×60mm Path B PASS rescinded — was MATC artifact (corrected T_MCU=84°C FAILS 80°C). |
...
| v1.1 full-load board sizing | `sim/gate12-v3-perbody` (sweep complete 2026-05-23) ⚠ **RESCINDED** — see `docs/THERMAL_ARCHITECTURE_DECISION.md` | ⚠ ~~Sweep 90×70 → 120×100. Smallest board with ≥5°C MCU margin = 105×85 mm (Tj_MCU = 73.98°C, +6.02°C margin). 100×80 falls short at +2.75°C. Above 105×85 the MCU is asymptotic ~74°C (heat-spreading length scale reached). Recommendation: adopt 105×85; LDO→buck NOT needed.~~ Recommendation invalidated 2026-05-23: actual-positions sweep gives MCU=82.46°C at 105×85 LDO, not 73.98°C. New sweep with actual positions + 3 architecture options (115×100 LDO / 105×85 buck / 110×90 buck) delivered in `THERMAL_ARCHITECTURE_DECISION.md`; Sai-pending pick between B and C (A ruled out). Sweep log: `sims/thermal-step4/runs/v11_sweep_2026-05-23.log`. |
