...
| BATT2 (secondary) voltage + current | **REMOVED** in Phase 2-exit 2026-05-20 (was: PA4 / PA7 ADC pin lines + `HAL_BATT2_VOLT_PIN 18` / `HAL_BATT2_CURR_PIN 7` defines + `HAL_BATT2_VOLT_SCALE 11.0`). novapcb v1 is single-Mauch per DECISIONS §5; #ifdef-guarded BATT2 runtime path was already inert (`BATT_MONITOR2 = 0` default). v2-dual-battery TBD (no commitment in DECISIONS or OPEN_QUESTIONS as of 2026-05-20). |
...
| `BATT2_VOLT_MULT` default | **REMOVED** in Phase 2-exit 2026-05-20 (was: `HAL_BATT2_VOLT_SCALE 11.0`). See BATT2 row above. |
