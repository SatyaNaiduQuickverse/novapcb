...
> ## ⚠ V2 SCOPE — DEFERRED FROM V1 (master 2026-05-24)
>
> This analysis was developed for novapcb v1 sub-step #102 but
> **deferred to v2** after two layout-attempt failures (Y=33 latitude
> hit PR #76 stitching-via conflicts; Y=45 latitude hit PR #77/#78
> route-crossing + edge-clearance conflicts). Both attempts produced
> 25-35 net new DRC violations from retroactive geometry-change vs
> already-routed dense topology.
>
> Per `DECISIONS.md §2.1` (added 2026-05-24): v1 is functional drop-in
> only; mechanical isolation including stress-relief slot moves to v2
> where FMU↔IMU board separation lets us plan the slot as a first-class
> constraint at routing time, not a retrofit.
>
> Analysis preserved here as v2 groundwork — geometry decisions (D
> U-opens-west), per-net detour plans, audit gate criteria, and the
> route-density lessons all apply when v2 takes this up.
>
> **Status (frozen as v2 reference)**: DRAFT — last v1 attempt
> reverted, all 2 commits' work preserved here.
>
> Branch `hw/imu-slot-polygon` off `sch/option-b-buck` head `ba7e28c`.
> Sub-step #102 (deferred from D placement to v2).
